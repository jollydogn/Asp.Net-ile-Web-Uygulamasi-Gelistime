...
namespace OtelRezervasyon
{
    public class DbProcess
    {
        ...
        {
            SqlCommand insert = new SqlCommand("insert into Musteri(Ad,Soyad,Eposta,CepTelefon,Cinsiyet,Notlar,KonukSayisi,Ulke,Uyruk) values(@Ad,@Soyad,@Eposta,@CepTelefon,@Cinsiyet,@Notlar,@KonukSayisi,@Ulke,@Uyruk)", con);
        ...
        {
            SqlCommand insert = new SqlCommand("insert into Rezervasyon(GelisAmaci,MusteriID,GelisTarihi,GidisTarihi,RezervasyonTarihi,RezervasyonKodu,OdaID,IndirimOrani) values(@GelisAmaci,@MusteriID,@GelisTarihi,@GidisTarihi,@RezervasyonTarihi,@RezervasyonKodu,@OdaID,@IndirimOrani)", con);
            ...
            insert.Parameters.AddWithValue("@RezervasyonTarihi", rezervasyonDegisken.rezervasyonTarihi);
        ...
        #region OdaTip
        public void OdaTipInsert(OdaTip odaTipDegisken)
        {
            SqlCommand insert = new SqlCommand("insert into OdaTip(OtelID,Ad,Aciklama,Fiyat,OdaKapasite) values(@OtelID,@Ad,@Aciklama,@Fiyat,@OdaKapasite)", con);
        ...
        {
            SqlCommand update = new SqlCommand("update OdaTip set OtelID=@OtelID,Ad=@Ad,Aciklama=@Aciklama,Fiyat=@Fiyat,OdaKapasite=@OdaKapasite where Ad=@DegisenAd", con);
        ...
        public List<Kullanicilar> KullanicilarSelect(Kullanicilar kullanici)
        {
            ...
            komut.Parameters.Add("@kad",kullanici.kullaniciAd);
            komut.Parameters.Add("@sifre", kullanici.sifre);
            SqlDataReader dr = komut.ExecuteReader();
            while (dr.Read())
            {
        ...
        public List<Konaklamalar> KonaklamalarSelect(Konaklamalar arananKonaklamalar)
        {
            ...
            con.Open();
            SqlCommand komut = new SqlCommand("select * from Konaklamalar where Ad like '"+ arananKonaklamalar.ad +"%' and IsActive=1", con);
            SqlDataReader dr = komut.ExecuteReader();
            while (dr.Read())
            {
        ...
        public List<Konaklamalar> odadaBulunanDigerKisileriGetir2(int odaNo,int tcKimlik)
        {
            List<Konaklamalar> result = new List<Konaklamalar>();
            con.Open();
            SqlCommand komut = new SqlCommand("Select * from Konaklamalar where OdaNo="+odaNo+" and TcKimlik<>"+tcKimlik+" and IsActive=1", con);
            komut.Parameters.AddWithValue("@tc", odaNo);
            komut.Parameters.AddWithValue("@odaNo", tcKimlik);
            SqlDataReader dr = komut.ExecuteReader();
            while (dr.Read())
            {
                Konaklamalar konaklamalar = new Konaklamalar(null)
        ...
        public List<Admin> AdminSelect(Admin adminGetir)
        {
            ...
            komut.Parameters.Add("@kad", adminGetir.kullaniciAd);
            komut.Parameters.Add("@sifre", adminGetir.sifre);
            SqlDataReader dr = komut.ExecuteReader();
            while (dr.Read())
            {
        ...
        public List<Odalar> OdaDurumSelect(string arananOda)
        {
            List<Odalar> result = new List<Odalar>();
            con.Open();
            SqlCommand komut = new SqlCommand("select * from OdaDurum where OdaNo like @ArananOda + '%'", con);
        ...
        public List<OdaOzellik> OdalarAnaSayfa(Rezervasyon rezervasyon,OdaTip odaKapasiteAra)
        {
            List<OdaOzellik> result = new List<OdaOzellik>();            
            con.Open();
            SqlCommand komut = new SqlCommand("select * from OdaTip inner join OdaOzellik on OdaTip.ID=OdaOzellik.TipID where OdaKapasite>=@Kapasite and OdaTip.ID in (select TipID from Odalar where ID not in (select OdaID from DolulukTakvimi, Odalar where(DolulukTakvimi.DoluTarih >= @GelisTarih and DolulukTakvimi.DoluTarih <= @GidisTarih)))", con);
            ...
            while (dr.Read())
            {
                ...
                OdaOzellik odaOzellik = new OdaOzellik(odaTip)
                {
                    ...
                    tipID = Convert.ToInt32(dr[7]),
                    klima = Convert.ToBoolean(dr[8]),
                    sauna = Convert.ToBoolean(dr[9]),
                    televizyon = Convert.ToBoolean(dr[10]),
                    manzaraId = Convert.ToInt32(dr[11]),
                    sacKurutmaMakinasi = Convert.ToBoolean(dr[12]),
                    jakuzi = Convert.ToBoolean(dr[13]),
                    sesYalitimi = Convert.ToBoolean(dr[14]),
                    mutfak = Convert.ToBoolean(dr[15]),
                    balkon = Convert.ToBoolean(dr[16])
                };
                result.Add(odaOzellik);
            }
            con.Close();
